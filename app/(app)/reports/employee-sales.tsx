...
import { useEmployeeSalesReport } from "../../../src/services/reports/queries";
...
export default function EmployeeSalesReport() {
  ...
  const { data = [], isLoading } = useEmployeeSalesReport(params);
  const topEmployees = useMemo(() => data.slice(0, 10), [data]);
  ...
  const barData = useMemo(
    () => ({
      labels: topEmployees.map((item) => formatShortLabel(item.name)),
      datasets: [
        {
          data: topEmployees.map((item) => item.total_sales ?? 0),
        },
      ],
    }),
    [topEmployees]
  ...
  const handleShare = async () => {
    const title = "Employee sales report";
    const range = `${fromDate} to ${toDate}`;
    const topLines = topEmployees
      .slice(0, 5)
      .map(
        (item, index) =>
            ...
            <View className="mt-3">
              {isLoading ? (
                Array.from({ length: 6 }).map((_, index) => (
                  <View key={`employee-skeleton-${index}`} className="mb-3">
